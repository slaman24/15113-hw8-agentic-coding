...
Error handling: Here are some error cases and how I would like them to be handled:
...
5. Based on filters that the user applies at the start, there are either no matching questions in the question bank or not as many as the user requested: If no questions, a friendly message should be printed and the user should be taken back to the starting screen to pick new filters. If not as many questions, a friendly message should be printed informing the user of this.
...
3. Users should be able to provide feedback on whether they like a question or not, and this should inform what questions they get next. After answering a question and seeing if they get it right or not as well as their current score, they should be prompted for user input to give feedback or can just hit 'Enter' to skip and move on to the next question.
...
5. Pressing 'q' or 'Q' at anytime should let the user be able to quit the quiz and the program should stop running. Before this though, users should be prompted with an 'Are you sure you want to quit' message so users can confirm their choice.
...
7. For scoring, users should get 5 points for each easy question they get right, 10 points for each medium questions they get right, and 15 points for each hard question they get right. There should also be streak scoring bonues where each question they get correct in a row, they should get 5 extra points, then 10, then 15 and so on. After they finish the round, they should see their final score as well as their high score that is saved for them in the system. A congratulations message should also be printed if they beat their high score.

8. When each question is displayed, it should say the level of difficulty and category, as well as what number it is. For example, if a user picked to answer 20 questions and this is the 12th question they are answering, it should say 12/20.
...
5. Users can optionally leave feedback on a question and based on this feedback and the level of difficulty and category of the question, this informs what next questions are asked (but still follow any filters the user selected at the start).
...
8. Users can identify if they have beaten their high score that is saved for them in the system and are congratulated for doing so.
